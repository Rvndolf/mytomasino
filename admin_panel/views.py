...
def admin_history(request):
    ...
    # Use select_related with nullable ticket (LEFT JOIN behavior via __isnull checks)
    ...
    # For deleted tickets (ticket=NULL), we check the saved category on the history entry itself
    if not request.user.is_superuser:
        staff_profile = StaffProfile.objects.filter(user=request.user).first()
        if staff_profile:
            allowed_categories = OFFICE_TICKET_CATEGORIES.get(staff_profile.office.name, [])
            # Include entries where:
            # - ticket still exists AND matches category, OR
            # - ticket was deleted BUT its saved category matches (ticket_category field)
            history_qs = history_qs.filter(
                Q(ticket__category__in=allowed_categories) |
                Q(ticket__isnull=True, ticket_category__in=allowed_categories)
            )
        else:
    ...
    if activity_type_filter:
        if activity_type_filter == 'created':
            ...
            # KEY FIX: deleted entries have ticket=NULL OR activity_type='deleted'
            history_qs = history_qs.filter(
    ...
    for entry in history_qs:
        ...
        if entry.activity_type:
            ...
            # If ticket is gone, it was deleted
        ...
        # Resolve ticket_id: use live ticket if exists, else fall back to
        # saved snapshot fields on the history entry
        ticket_id = None
        if entry.ticket:
            ticket_id = entry.ticket.id
        elif hasattr(entry, 'deleted_ticket_id') and entry.deleted_ticket_id:
            ticket_id = entry.deleted_ticket_id
